...
import { 
  Calendar, Building2, Star, ClipboardList, Clock, 
  Search, Filter, MoreHorizontal, Eye, XCircle, CreditCard
} from 'lucide-react';
import { 
...
export default function Bookings({ auth, bookings }) {
    ...
    const handleCancel = (bookingId) => {
        if (confirm('Are you sure you want to cancel this booking?')) {
            router.patch(route('booking.update-status', bookingId), { status: 'cancelled' });
        }
    };
    ...
    return (
        <AuthenticatedLayout>
            ...
            <DashboardShell
                title="My Bookings"
                subtitle="Manage your service appointments and history."
                action={{
            ...
            >
                <div className="space-y-4">
                    ...
                    {/* Bookings Table */}
                    <DataCard title={`${statusFilter === 'all' ? 'All' : statusFilter.charAt(0).toUpperCase() + statusFilter.slice(1)} Bookings`} icon={ClipboardList}>
                        <div className="-mx-6">
                            <Table>
                                <TableHeader>
                                    <TableRow className="bg-muted/30 hover:bg-muted/30">
                                        <TableHead className="pl-6 py-3 uppercase text-[10px] font-bold tracking-widest text-muted-foreground">Booking Info</TableHead>
                                        <TableHead className="py-3 uppercase text-[10px] font-bold tracking-widest text-muted-foreground">Provider</TableHead>
                                        <TableHead className="py-3 uppercase text-[10px] font-bold tracking-widest text-muted-foreground">Schedule</TableHead>
                                        <TableHead className="py-3 uppercase text-[10px] font-bold tracking-widest text-muted-foreground">Price</TableHead>
