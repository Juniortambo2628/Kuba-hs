      ...
      {pageSettings.map((setting) => (
        <div key={setting.id} className="min-w-0">
          {setting.type === 'image' ? (
            ...
            <Card className={cn(dashboardUi.card.padding, 'border border-border/40 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm space-y-3 hover:border-primary/20 transition-all h-full overflow-hidden min-w-0')}>
