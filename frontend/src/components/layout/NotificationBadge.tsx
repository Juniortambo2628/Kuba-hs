...
export function NotificationBadge() {
  ...
  useEffect(() => {
    // Optionally pre-load a subtle ping sound if desired.
    audioRef.current = new Audio("/assets/notification.mp3"); // Optional
  }, []);
  ...
  useEffect(() => {
    ...
    if (echo) {
      // Listen to the private User channel for BroadcastNotifications
  ...
  const handleNotificationClick = async (notification: Notification) => {
    ...
    // Navigate — sanitize URL to ensure it's a relative frontend path
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-80 sm:w-96 rounded-2xl shadow-xl border-gray-100 dark:border-white/5 p-0 overflow-hidden bg-white dark:bg-[#0f1423]">
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start w-full gap-2 mb-1">
                      <p className={`text-sm tracking-tight ${isUnread ? "font-black text-gray-900 dark:text-white" : "font-bold text-gray-700 dark:text-gray-300"}`}>
