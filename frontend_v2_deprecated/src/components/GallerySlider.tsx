...
export default function GallerySlider({
  ...
  const finalImages = React.useMemo(() => {
    if (!galleryImgs || galleryImgs.length === 0) {
      return ["/placeholder-light.png"];
    }
    const filtered = galleryImgs.filter((img) => {
      if (!img) return false;
      if (typeof img === "string" && img.trim() === "") return false;
      return true;
    });
    return filtered.length > 0 ? filtered : ["/placeholder-light.png"];
  }, [galleryImgs]);

  // Ensure index is within bounds of finalImages
  const safeIndex = (index >= 0 && index < finalImages.length) ? index : 0;
  const currentImage = finalImages[safeIndex];
  
  // Extra layer of protection: Ensure imageSrc is never an empty string, null, or undefined
  const getValidSrc = () => {
    if (!currentImage) return "/placeholder-light.png";
    if (typeof currentImage === "string") {
      return currentImage.trim() === "" ? "/placeholder-light.png" : currentImage;
    }
    // If it's an object (StaticImageData), ensure it has a src
    if (typeof currentImage === "object" && !('src' in currentImage)) {
      return "/placeholder-light.png";
    }
    return currentImage;
  };

  const imageSrc = getValidSrc();
  ...
  // In Chisfis, we can handle the dark mode placeholder switching if we want, 
  // but for now let's just use the light one as default since Image component handles it.
  // Actually, we can use a small hack to show different placeholders for dark/light if we use a component.
